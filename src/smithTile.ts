...
function transformPatch(
    ...
    return {
        ...
        sockets: patch.sockets.map((socket) =>
            transformPort(transform, socket)
        ),
...
function composeI(S: SmithPatch, N: SmithPatch, Aa: SmithPatch, Ab: SmithPatch, Ma: SmithPatch): SmithPatch {
...
function nextArticulatedLevel(prev: ArticulatedLevel): ArticulatedLevel {
...
export function patchColorGroups(patch: SmithPatch): PatchColorGroup[] {
    const components =
        patch.components && patch.components.length > 0 ? patch.components : [patch];
...
export const PATCHES = { S0, N0, Aa0, Ab0, Ma0, Mb0,
    I1: level1.I!,
    S1: level1.S,
    N1: level1.N,
    Aa1: level1.Aa,
